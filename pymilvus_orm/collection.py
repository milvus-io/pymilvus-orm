...
class Collection(object):
    ...
    def __init__(self, name, data=None, schema=None, **kwargs):
        ...
        """
        # TODO: while data = None? schema = None?
        self._name = name
        self._kwargs = kwargs
        ...
        if has:
            ...
            # TODO: construct by describe collection result
            fields = []
            for field in resp.fields:
                fields.append(FieldSchema(name=field.name, dtype=field.type, description=field.description,
                                          is_primary=field.is_primary_key))
            server_schema = CollectionSchema(fields=fields, description=resp.description, **resp)
            if schema is None:
                self._schema = server_schema
                # No need to check data, insert method will check(can't insert data when create collection).
                if data is not None:
        ...
        if schema is not None:
            ...
            if isinstance(schema, CollectionSchema):
                for i in range(len(schema.fields)):
                    if isinstance(schema.fields[0], FieldSchema):
                        schema.fields[0] = schema.fields[0].__dict__
                    else:
